...
./api_new.py --hostname=$HOSTNAME --username=$USERNAME --password=$PASSWORD --check=$TYPE_OF_CHECK --metric=$METRIC --node="$NODE" --filters="$FILTER" --conf=$CONF   --zabbix_server=$ZABBIX_SERVER --senderhostname=$ZABBIX
